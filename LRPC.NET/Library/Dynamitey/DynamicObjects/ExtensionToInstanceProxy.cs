    ...
    public class ExtensionToInstanceProxy: BaseForwarder
    {
        ...
        public class Invoker:BaseObject
        {
            ...
            public override bool TryInvoke(InvokeBinder binder, object[] args, out object result)
            {
                ...
                {
                    tArgs = Enumerable.Select(OverloadTypes[args.Length].Zip(args, Tuple.Create),
                        it => it.Item2 != null ? Dynamic.InvokeConvert(it.Item2, it.Item1, @explicit: true) : null).ToArray();
            ...
            {
                result = new Invoker(Name, GenericParams, Enumerable.Select(indexes, it => Dynamic.InvokeConvert(it, typeof(Type), @explicit: true)).Cast<Type>().ToArray(), Parent);
                return true;
            }
        }
        ...
        public class OverloadInvoker:Invoker
        {
            ...
            public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
            {
                result = new Invoker(Name, GenericParams, GenericMethodParameters, Parent, Enumerable.Select(indexes, it => Dynamic.InvokeConvert(it, typeof(Type), @explicit: true)).Cast<Type>().ToArray());
                return true;
            }
        }
